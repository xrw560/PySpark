...
dt_regressor = DecisionTreeRegressor().setLabelCol("indexedLabel").setFeaturesCol("indexedFeatures")
# 在pipeline中进行设置
pipeline_classifier = Pipeline().setStages([label_indexer, feature_indexer, dt_regressor, label_converter])
# 进行决策树模型
...
"""5 评估决策树分类模型"""
evaluator_regressor = RegressionEvaluator().setLabelCol("indexedLabel").setPredictionCol("prediction").setMetricName("rmse")
rmse = evaluator_regressor.evaluate(predictions_classifier)
